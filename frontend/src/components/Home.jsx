...
function Home() {
    ...
    const todoCreate= async ()=>{
         if (!newTodo) return; /// if nothing in todo , means no text then no need to create a todo even though he press add todo button
        try {
  ...
  return (
    <div className=' my-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6'>
      ...
      <button  
      onClick={()=> logout()}
      className='mt-6 px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-800 duration-500 mx-auto block'>Logout</button>
    </div>
  )
}
